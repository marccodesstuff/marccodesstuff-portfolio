...
const TactileHero = ({ 
  ...
  useEffect(() => {
    ...
    // Renderer with industrial aesthetics
    ...
    // Create industrial product schematic (abstract modular synth module)
    createProductSchematic(scene);

    // Add lighting - dramatic studio lighting
    setupLighting(scene);
    ...
    const animate = () => {
      ...
      // Subtle rotation for wireframe mode
  ...
  // Create abstract modular synth module schematic
  const createProductSchematic = (scene: THREE.Scene) => {
    const group = new THREE.Group();
    
    // === MODULE BODY (Industrial chassis) ===
    ...
    // === VIRTUAL KNOB GROUP (Interactive controls) ===
    const knobsGroup = new THREE.Group();
    
    // Create 4 rotary encoder-style knobs
    for (let i = 0; i < 4; i++) {
      ...
      knob.rotation.x = Math.PI / 2; // Align cylinder to rotate in Y
      knob.position.set(x, 0, z + 0.43); // Slightly in front of panel
      
      // Add groove pattern (wireframe ring)
    ...
    // Add text label to screen (wireframe mesh approximating letters)
    addWireframeText(screen, 'TE-2X');

    // === I/O PORTS (Right side) ===
    const ioPortsGroup = new THREE.Group();
    
    // Create 8 input/output jacks
    for (let i = 0; i < 8; i++) {
      ...
      jack.position.set(x, y, 1.75); // On the side
      
      // Add ring around each jack
    ...
    ioPortsGroup.rotation.y = -Math.PI / 2; // Face right side
    group.add(ioPortsGroup);

    // === KNOB INDICATORS (LED-style rings) ===
    const indicatorMaterials = [
      new THREE.MeshBasicMaterial({ color: 0x393939 }),
      new THREE.MeshBasicMaterial({ color: 0xff6b1a }), // Orange for active
      new THREE.MeshBasicMaterial({ color: 0x393939 }),
      new THREE.MeshBasicMaterial({ color: 0xffff3c }), // Yellow easter egg
    ];
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // Ring geometry for LED indicator
    ...
    ;(group as any).__schematic = true;
  };

  // Create dramatic studio lighting
  const setupLighting = (scene: THREE.Scene) => {
    ...
    // Grid helper for blueprint aesthetic
    const gridHelper = new THREE.GridHelper(10, 10, 0x333333, 0x1a1a1a);
    gridHelper.position.y = -1.25;
    scene.add(gridHelper);
  };

  // Add wireframe text approximation to screen
  const addWireframeText = (screen: THREE.Mesh, text: string) => {
    ...
    // Draw wireframe-style text outline
  ...
  return (
    ...
    >
      ...
      {/* View mode controls - Virtual switches */}
      ...
      {/* Easter egg: Hidden toggle (click anywhere on canvas for bonus mode) */}
      <canvas 
        ref={(node) => {
